(function () {
    ...
    [Select, Insert, Update, Delete].forEach(function (stmt) {
        stmt.defineClause = function (clause_id, template, opts) {
            ...
            if (typeof templ_fn != 'function')
                templ_fn = function (opts) {
                    return templ(template, this, opts);
                };
    ...
    sql.and = function and() {
        return new Group('AND', argsToArray(arguments));
    };
    sql.or = function or() {
        return new Group('OR', argsToArray(arguments));
    };
    ...
    sql.like = function like(col, val, escape_char) {
        return new Like(col, val, escape_char);
    };
    ...
    sql.between = function between(col, val1, val2) {
        return new Between(col, val1, val2);
    };
    ...
    sql.isNull = function isNull(col) {
        return new Unary('IS NULL', col);
    };
    sql.isNotNull = function isNotNull(col) {
        return new Unary('IS NOT NULL', col);
    };
    ...
    sql.conversions = {
        ...
        'Date': function (dt) {
            return "TIMESTAMP WITH TIME ZONE '" + dt.toISOString().replace('T', ' ').replace('Z', '+00:00') + "'";
        },
        'Array': function (arr) {
            return '{' + arr.map(sql.convert).join(', ') + '}';
        }
    };

    function handleTables(tables, opts) {
        return tables.map(function (tbl) {
            return handleTable(tbl, opts);
        }).join(', ');
    }
    ...
    templ.helpers = {
        'if': function (val, opts, contents, ctx) {
            return val ? templ(contents, ctx, opts) : '';
        },
        'ifNotNull': function (val, opts, contents, ctx) {
            return val != null ? templ(contents, ctx, opts) : '';
        },
