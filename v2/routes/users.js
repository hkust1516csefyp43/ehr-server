...
/*PUT*/
router.put('/change_info/:id', function (req, res) {
  var sent = false;
  var params = {};
  var param_headers = req.headers;
  var body = req.body;
  ...
  if (valid.empty_object(body)) {
    ...
    db.check_token_and_permission("users_read", token, function (err, return_value, client) {
      if (!return_value) {                                        //return value == null >> sth wrong
        ...
        if (return_value.user_id !== req.params.id) {
          res.status(errors.no_permission()).send('No permission to edit other user');
        } else if (return_value.expiry_timestamp < Date.now()) {
          res.status(errors.access_token_expired()).send('Access token expired');
        } else {
